import {deleteImage, PROJECT, storage, upload} from "../firebase/uploadImage.js";
...
$('#btn-op-upload').on('click', function () {
    Swal.fire({
        ...
        willClose: () => {
            ...
            if ($('#uploadImg').val() != '') {
                deleteImage($('#uploadImg').val(), PROJECT).then(result => {
                    if (result) {
...
$(document).ready(function () {
    $('select#services').change(function () {
        $('#serviceValue').val($(this).val())
        console.log($('#serviceValue').val())
    })

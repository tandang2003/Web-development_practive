...
<script>
    ...
    validate('#form-add-user', adminAddUserValidator, function (form) {
        $.ajax({
            ...
            success: function (data) {
                if (data.status === 200) {
                    autoCloseAlertWithFunction(data.message, 1500, swal2Icon.success, () => {
                        window.location.href = data.data;
                    })
                } else {
                    autoCloseAlert(data.message, 1500, swal2Icon.error)
                }
            },
            error: function () {
                errorAlert("Hệ thống đang gặp sự cố")
            }
        })
    })
</script>

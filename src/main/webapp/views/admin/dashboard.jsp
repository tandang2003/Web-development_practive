                    ...
                    },
                    {
                        ...
                        action: function () {
                            ...
                            if (selectedRows.length > 3) {
                                alert('Không thể xóa đồng thời nhiều hơn 3.', 'warning');
                                return;
                            }
                            if (selectedRows.length === 0) {
                                alert('Không có dữ liệu nào được chọn', 'warning');
                                return;
                            }
                            confirmAlert('Bạn có chắc chắn muốn xóa dữ liệu đã chọn?', function () {
                                // Use ajax to delete data
                                $.ajax({
                                    url: "${pageContext.request.contextPath}/api/admin/logs/delete",
                                    type: "DELETE",
                                    data: JSON.stringify(selectedRows.map(row => row.id)),
                                    contentType: "application/json",
                                    success: function () {
                                        alert('Xóa dữ liệu thành công', 'success');
                                        table.ajax.reload();
                                    },
                                    error: function (xhr, status, error) {
                                        console.log('Error deleting rows:', error);
                                        alert('Xóa dữ liệu thất bại', 'error');
                                    }
                                });
                            });
                        }
